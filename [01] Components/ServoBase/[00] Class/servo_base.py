...
class ServoBase:
    """This serves as a base class for servo control. The goals are to:
     1. Allow the use of logical angles that make sense in the application, rather than the physical angles of the servo.
     2. Specify movement over a given amount of time, rather than just a single step.
    A start and an end angle are provided. Note that these angles are not necessarily in order. (angle_start may be > angle_end)
    ...
    def __init__(self, 
        ...
        """Set up a servo on the given pin with the specified angles and parameters.

        Args:
            pin (int): Pin number for the servo PWM signal
            angle_start (float): Start angle for the servo (deg)
            angle_end (float): End angle of the servo (deg)
            min_us (float, optional): PWM parameter, microseconds. Defaults to 544.0.
            max_us (float, optional): PWM parameter, microseconds. Defaults to 2400.0.
            min_deg (float, optional): Minimum angle of the servo (deg). Defaults to 0.0.
            max_deg (float, optional): Maximum angle of the servo (deg). Defaults to 180.0.
            freq (int, optional): PWM frequency, Hz. Defaults to 50.
        """
    ...
    def write(self, deg: float) -> None:
        """Command the servo to move to the given angle
